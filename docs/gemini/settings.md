...
This document provides research findings on optimal Gemini CLI configuration for **performance and efficiency**. The goal is to minimize latency, reduce token usage, optimize costs, and create a fast, responsive development experience.

## Key Performance & Efficiency Considerations

### 1. Model Selection (`model.name`)
...
**Why:**
- Fastest response times among Gemini models
- Lower latency compared to Pro variants
- Still provides 1M token context window
- Optimized for real-time interactions
- Free tier: 60 requests/min, 1,000 requests/day

**Alternative:** `gemini-2.5-pro` for complex reasoning tasks (slower but more capable)

### 2. Shell Output Optimization (`model.enableShellOutputEfficiency`)

**Performance Impact:** High  
**Default:** `true` (keep enabled)
...
**Why:**
- Automatically summarizes large shell command outputs
- Reduces token consumption from verbose tool outputs
- Prevents context window bloat
- Maintains essential information while cutting noise

### 3. Token Caching (Automatic)

**Performance Impact:** Very High  
**Cost Impact:** Significant savings

**Key Facts:**
- Automatically enabled for API key users
- Not available for OAuth users (Code Assist API limitation)
- Reuses previous system instructions and context
- Check savings with `/stats` command

**Note:** Currently using OAuth authentication (`oauth-personal`), so caching is unavailable. Consider switching to API key authentication for better performance.

### 4. UI Optimization for Speed

**Performance Impact:** Medium  
**Recommendation:** Minimal UI for fastest experience
...
- Faster startup time
- Less visual clutter for focused work
- Keep `hideContextSummary: false` to stay aware of loaded context

### 5. Tool Output Truncation

**Performance Impact:** High  
**Default Settings:** Well-optimized
...
**Why:**
- Prevents massive outputs from consuming context window
- `ripgrep` is much faster than fallback implementation
- Threshold of 20K characters is reasonable for most use cases

### 6. Context Management

**Performance Impact:** High  
**Recommendation:** Intentional context loading
...
**Why:**
- Limit context discovery to avoid scanning excessive directories
- `respectGitIgnore` prevents scanning `node_modules`, build artifacts
- `loadFromIncludeDirectories: false` keeps context focused on current directory
- Smaller context = faster processing

### 7. Session Management

**Performance Impact:** Medium  
**Recommendation:** Limit session history
...
- Automatic compression at 70% threshold
- 20 turns is usually sufficient for most sessions
- Use `/compress` manually if needed earlier

### 8. Telemetry (Performance Monitoring)

**Performance Impact:** Minimal  
**Recommendation:** Enable for insights
...
**Why:**
- Minimal overhead for local logging
- Track tool performance and API latency
- Identify bottlenecks in your workflow
- `logPrompts: false` for privacy

### 9. Auto-Accept Safe Tools

**Performance Impact:** High (UX)  
...
**Why:**
- Eliminates confirmation dialogs for safe operations
- Faster workflow for read-only commands
- Reduces cognitive interruptions
- Still requires confirmation for write operations

### 10. Disable Expensive Features
...
**Why:**
- Prompt completion adds AI inference overhead
- Checkpointing requires disk I/O and processing
- Disable if not actively using these features

## Recommended Optimal Configuration

Here's a complete `settings.json` optimized for performance and efficiency:
...
**Performance Impact:** Variable (depends on server)

Only configure MCP servers you actively use. Each server adds discovery overhead.
...
**Tips:**
- Set `trust: true` for servers you trust to skip confirmations
- Use `includeTools` to limit which tools are exposed
...
- Use hierarchical structure (global → project → local)
- Avoid duplicating information across files
- Use `@imports` to modularize large context files

**Example Efficient GEMINI.md:**
```markdown
# Project Context

## Tech Stack
TypeScript, Node.js 20+, React 18
...
**Performance Impact:** High (if enabled)
...
**Recommendation:**
- Disable sandbox for trusted projects (faster execution)
- Enable with `--sandbox` flag only when testing untrusted code
- Use Docker sandbox for best isolation when needed
...
### Speed vs. Safety
- Enabling `tools.autoAccept` is faster but less safe
- Consider project sensitivity when configuring

### Speed vs. Quality
- `gemini-2.5-flash` is faster but may be less accurate for complex tasks
- Switch to `gemini-2.5-pro` for critical reasoning

### Speed vs. Features
- Disabling checkpointing, prompt completion, etc. improves performance
- Re-enable features when specifically needed

### Token Caching vs. OAuth
- API key authentication enables token caching (significant savings)
- OAuth is more convenient but lacks caching support
- Consider switching to API key for heavy usage
...
1. **Startup Time:**
   ```bash
   time gemini -p "hello" --output-format json
   ```
...
- Minimal (absolute minimum for speed)

## Future Optimizations

**Watch for in future releases:**
- Token caching support for OAuth users
- Streaming improvements for faster perceived latency
- Model router for automatic model selection
- Better prompt caching strategies

## References

- [Official Configuration Docs](https://raw.githubusercontent.com/google-gemini/gemini-cli/main/docs/get-started/configuration.md)
- [Token Caching](https://raw.githubusercontent.com/google-gemini/gemini-cli/main/docs/cli/token-caching.md)
- [Sandboxing](https://raw.githubusercontent.com/google-gemini/gemini-cli/main/docs/cli/sandbox.md)
- [Telemetry](https://raw.githubusercontent.com/google-gemini/gemini-cli/main/docs/cli/telemetry.md)
...
1. Review current `settings.json` configuration
2. Apply recommended optimizations incrementally
3. Benchmark performance improvements
4. Fine-tune based on your specific workflow
5. Consider switching to API key authentication for token caching
